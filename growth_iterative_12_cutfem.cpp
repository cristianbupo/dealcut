/**
 * @brief Iterative growth/ossification with CutFEM-Library — SOC-12 geometry.
 *
 * SOC-12: concave/bilobed top (p2_geom = -0.6), interfaceY = 0.75,
 *         wider lateral bulge (p1_geom = 1.15), BSpline weights {1,2,2,1,1,1,2,2,1}.
 *
 * Single-run iterative workflow:
 ...
 *   Iteration 2+: prev ossified becomes bone, new ossified from new MI
 *
 * Materials:
 *   bone      (y < 0.75):           E=500,  ν=0.2
 *   cartilage (y ≥ 0.75, not oss):  E=6,    ν=0.47
...
static constexpr double p1_geom = s *  1.15;   // wider lateral bulge
static constexpr double p2_geom = s * -0.6;    // concave/dipped top
...
// Material (plane strain) — interface at y=0.75
static constexpr double interfaceY = s * 0.75;
...
static constexpr double kMi = 1.0;
static constexpr double oss_spline_band = 0.3;

// Load — SOC-12 uses du_frac=0.03, u_radius=3*du
static constexpr double load_center_u_frac = 0.50;
static constexpr double load_du_frac       = 0.03;
...
// ============================================================
// Gmsh geometry — SOC-12 (concave top, wider lateral bulge)
// ============================================================
static void build_soc_geometry() {
    gmsh::initialize();
    gmsh::model::add("growth_iterative_12_cutfem");
...
// ============================================================
// Load steps — SOC-12: du_frac=0.03, u_radius=3*du
// ============================================================
static std::vector<StepDef> build_default_steps() {
    ...
    double overlap_radius = 3.0 * du;
    std::vector<StepDef> steps = {
        {uc+2*du, overlap_radius, 0.50*load_p_peak},
        {uc+du,   overlap_radius, 0.75*load_p_peak},
        {uc,      overlap_radius, 1.00*load_p_peak},
        {uc-du,   overlap_radius, 0.75*load_p_peak},
        {uc-2*du, overlap_radius, 0.50*load_p_peak}
    };
...
int main(int argc, char **argv) {
    ...
    std::filesystem::create_directories("output_growth_iterative_12");
    build_soc_geometry();
                                ...
                                const std::vector<double> &phi_oss_data)
    {
        ...
        for (unsigned int sidx = 0; sidx < result.all_sols.size(); ++sidx) {
            ...
            fct_t uh(Wh, sp);
            Paraview<mesh_t> w(Khi, "output_growth_iterative_12/growth_" + tag + "-step-" + std::to_string(sidx+1) + ".vtk");
        ...
        {
            Paraview<mesh_t> w(Khi, "output_growth_iterative_12/growth-fields_" + tag + ".vtk");
        ...
        if (EXPORT_ACTIVE) {
            Paraview<mesh_t> w;
            w.writeActiveMesh(Khi, "output_growth_iterative_12/growth-active_" + tag + ".vtk");
            w.add(const_cast<fct_t&>(uh_avg), "displacement", 0, 2);
            w.add(const_cast<fct_t&>(hd_fh), "hydrostatic", 0, 1);
            w.add(const_cast<fct_t&>(oct_fh), "oct_shear", 0, 1);
            w.add(const_cast<fct_t&>(mi_fh), "miner_index", 0, 1);
        ...
        write_trimmed_both_vtk(
            "output_growth_iterative_12/growth-trimmed_" + tag + ".vtk", Khi,
            clip_levelsets, sca,
        ...
        if (EXPORT_CARTILAGE) {
            ...
            Paraview<mesh_t> w(Khi_cart, "output_growth_iterative_12/growth-cartilage_" + tag + ".vtk");
            w.add(const_cast<fct_t&>(hd_fh), "hydrostatic", 0, 1);
            w.add(const_cast<fct_t&>(oct_fh), "oct_shear", 0, 1);
            w.add(const_cast<fct_t&>(mi_fh), "miner_index", 0, 1);
            w.add(const_cast<fct_t&>(mat_fh), "material", 0, 1);
    ...
    std::vector<double> prev_mi_avg(nb_sca, 0.0);
    ...
    std::cout << "\nDone. Output in output_growth_iterative_12/\n";
    return 0;
}

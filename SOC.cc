...
// The loading process is kept continuous in u, but assembled in a
// SOC-11-like way: local active window, local normalized coordinate,
...
namespace SOC
{
  ...
  private:
    ...
    LameParameters lame_at_point(const Point<dim> &p) const;
  ...
  template <int dim>
  LameParameters ImmersedElasticity<dim>::lame_at_point(const Point<dim> &p) const
  {
  ...
  void ImmersedElasticity<dim>::assemble_stiffness()
  {
    ...
    flags.inside  = update_gradients | update_JxW_values | update_quadrature_points;
    flags.surface = update_values | update_gradients | update_JxW_values | update_quadrature_points;
            ...
            for (const unsigned int q : inside->quadrature_point_indices())
              {
                ...
                const auto lame = lame_at_point(xq);
  ...
  // Continuous u-parametric load, assembled in a SOC-11-like way.
  template <int dim>
        ...
        for (const unsigned int q : surface->quadrature_point_indices())
          {
            ...
            // Same idea as SOC-11: project once, keep only the local active window,
            ...
            // SOC-11-like segment gating: only segments overlapping the active window matter.
        ...
        if (!is_cut)
          {
            ...
            cell->set_material_id(is_top ? 3 : 0);
          }
        else
          {
            const bool cell_is_top = (cell->center()[1] >= interfaceY);
            cell->set_material_id(cell_is_top ? 2 : 1);
          }

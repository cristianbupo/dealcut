...
namespace Step85
{
  ...
  // The analytical solution to the Poisson problem reads
  // u(x) = 1 - (2/dim)*(||x||^2 - 1)
  template <int dim>
  ...
  template <int dim>
  void LaplaceSolver<dim>::output_results(const unsigned int cycle) const
  {
    std::cout << "Writing vtu file for cycle " << cycle << std::endl;

    // Interpolate exact solution onto the same DoFHandler
    Vector<double> exact(solution.size());
    VectorTools::interpolate(dof_handler, AnalyticalSolution<dim>(), exact);

    DataOut<dim> data_out;
    data_out.add_data_vector(dof_handler, solution, "solution");
    data_out.add_data_vector(dof_handler, exact, "exact_solution");
  ...
  // error in the solution in the $L^2$-norm.

    ...
    const cutmesh_t &Khi, const mesh_t &Kh, int nb_sca_dof)
{
  ...
  for (int ka = 0; ka < nact; ++ka) {
    ...
    if (!has_bone) {
      ...
      ElementInterfaceLS phi_interface;
      phi_interface.xmin = xmin; phi_interface.xmax = xmax;
      phi_interface.ymin = ymin; phi_interface.ymax = ymax;
      phi_interface.v00 = interfaceY - pts[0].y;
      phi_interface.v10 = interfaceY - pts[1].y;
      phi_interface.v11 = interfaceY - pts[2].y;
      phi_interface.v01 = interfaceY - pts[3].y;
...
int main(int argc, char **argv) {
  ...
  // if (run_mode == RunMode::export_mi) {
  {
    ...
    std::vector<double> phi_outer_data(nb_sca), phi_interface_data(nb_sca);
    for (int k = 0; k < Kh.nt; ++k) {
      const auto &FK = Sh[k];
      for (int j = 0; j < FK.NbDoF(); ++j) {
        ...
        phi_interface_data[iglo] = P.y - interfaceY;
      }
    }
    // Combined level set: R-function union of the two curves.
    // phi_combined = min(phi_outer, phi_interface)
    ...
    std::span<double> phi_interface_span(phi_interface_data);
    fct_t phi_interface_fh(Sh, phi_interface_span);
  ...
  // Poisson equation on SOC cartilage domain (phi_combined region)
  {
  std::filesystem::create_directories("output/step85_cutfem");

  // Build domain level set: phi_domain < 0 inside cartilage region
  // (inside SOC polygon AND above bone/cartilage interface) = -phi_combined
  std::vector<double> phi_domain_data(nb_sca);
  for (int k = 0; k < Kh.nt; ++k) {
    const auto &FK = Sh[k];
    for (int j = 0; j < FK.NbDoF(); ++j) {
      int iglo = Sh(k, j);
  ...
  fct_t phi_domain_fh(Sh, phi_domain_span);
  InterfaceLevelSet<mesh_t> domain_interface(Kh, phi_domain_fh);

  cutmesh_t Khi_p(Kh);
  Khi_p.truncate(domain_interface, 1);

  space_t Vh_p(Kh, DataFE<mesh_t>::P1);
  cutspace_t Wh_p(Khi_p, Vh_p);

  std::cout << "Poisson on SOC cartilage: h = " << h
            << ", DOFs = " << Wh_p.get_nb_dof() << "\n";

  CutFEM<mesh_t> poisson(Wh_p);

  funtest_t u(Wh_p, 1), v(Wh_p, 1);

  const double nitsche_parameter = 5.0 * 2.0 * 1.0;
  ...
  // Bulk RHS: ∫_Ω f v dx
  fct_t fRhs(Wh_p, [](R2 P, int, int) -> double { return 4.0; });
  poisson.addLinear(
      innerProduct(fRhs.expr(), v),
      Khi_p);

  // Nitsche RHS: -∫_Γ g(n·∇v) + (σ/h)∫_Γ g v  (g=1 on boundary)
  fct_t gD(Wh_p, [](R2 P, int, int) -> double { return 0.0; });
  poisson.addLinear(
  ...
  std::span<double> data_uh(poisson.rhs_.data(), Wh_p.get_nb_dof());
  fct_t uh(Wh_p, data_uh);

  Paraview<mesh_t> writer(Khi_p, "output/step85_cutfem/step85_cutfem_soc.vtk");
  writer.add(uh, "solution", 0, 1);
  writer.add(phi_domain_fh, "phi_domain", 0, 1);

  // Extract Poisson nodal values onto Sh grid
  std::vector<double> uh_nodal(nb_sca, 0.0);
  for (int ka = 0; ka < Khi_p.get_nb_element(); ++ka) {
    int kb = Khi_p.idxElementInBackMesh(ka);
    const auto &FK = Sh[kb];
    for (int j = 0; j < FK.NbDoF(); ++j) {
      int iglo = Sh(kb, j);
      if (iglo < 0 || iglo >= nb_sca) continue;
      R2 P = FK.Pt(j);
      uh_nodal[iglo] = uh.evalOnBackMesh(kb, 0, &P.x, 0, 0);
    }
  }
  double uh_max = *std::max_element(uh_nodal.begin(), uh_nodal.end());
  std::cout << "Poisson max = " << uh_max << "\n";
    ...
    fct_t lambda_fh = make_base_lambda();
    {
      ...
      for (int k = 0; k < Kh.nt; ++k) {
        ...
        for (int j = 0; j < FK.NbDoF(); ++j) {
          int iglo = Sh(k, j);
          if (iglo < 0 || iglo >= nb_sca) continue;
          if (uh_nodal[iglo] >= threshold && FK.Pt(j).y >= interfaceY) {
            tmu_v[iglo] = 2.0 * mu_oss;
            lam_v[iglo] = lambda_oss;
            ++n_oss;
          }
      ...
      std::cout << "  Ossified nodes: " << n_oss << " / " << nb_sca << "\n";
    }

    auto result = run_elasticity(two_mu_fh, lambda_fh);

    // Build phi_oss (positive = ossified) and material indicator fields
    std::vector<double> phi_oss_data(nb_sca), mat_data(nb_sca, 0.0);
    for (int k = 0; k < Kh.nt; ++k) {
      const auto &FK = Sh[k];
      for (int j = 0; j < FK.NbDoF(); ++j) {
        int iglo = Sh(k, j);
        if (iglo < 0 || iglo >= nb_sca) continue;
        R2 P = FK.Pt(j);
        phi_oss_data[iglo] = uh_nodal[iglo] - threshold;
        if      (P.y < interfaceY)             mat_data[iglo] = 0.0; // bone
        else if (phi_oss_data[iglo] >= 0.0)    mat_data[iglo] = 2.0; // ossified
        else                                   mat_data[iglo] = 1.0; // cartilage
      }
    }
